"""
Changes: cost for min conc
inititlisation strategy -- now random search before NM.
...
if __name__ == "__main__":
    ...
    def run_simulation(log10_fit_params, logger):
        ...
        polarisation_g4 = sim_values_polarisation["p_t"][4:, :, -1].sum() / sim_values_polarisation["p_t"][:, :,
                                                                            -1].sum() * preNEBD_membrane_frac
        postNEBD_g4 = sim_values_postNEBD["p_t"][4:, :, -1].sum() / sim_values_postNEBD["p_t"][:, :,
                                                                    -1].sum() * postNEBD_membrane_frac
        ...
        cost_dict["ASI"] = np.nansum(np.array([np.abs(c_pol[:-12:12] - asi_norm.reshape(4, -1)[i]) ** 2 for i, c_pol in
