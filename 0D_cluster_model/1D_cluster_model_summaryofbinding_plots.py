...
"""
A reasonable parameter scan would be: 
k_bind
k_seq
B_tot
...
k_bind_c_range = np.logspace(-2,0.5,10)
p_distribution_range = np.zeros((len(k_bind_c_range),n_clust))
for i, k_bind_c in enumerate(k_bind_c_range):
    _param_values = param_values_initialise.copy()
    _param_values["kbind_m"] = k_bind_c

    _param_values["kbind_c"] = k_bind_c*3
    _params = [_param_values[nm] for nm in param_names]

    sol_init = solve_ivp(model, tspan_init, y0, method="LSODA", t_eval=teval_init, jac=jac, args=(_params,))

    _y = sol_init.y.reshape(param_values_initialise["n_clust"] * 2 + 1, 2, -1)
    p_t, b_t = _y[:n_clust], _y[n_clust:]

    p_distribution_range[i] = p_t[:,0,-1]
    print(i)

cols = plt.cm.plasma(np.linspace(0,0.7,len(k_bind_c_range)))
fig, ax = plt.subplots(len(k_bind_c_range),figsize=(4,6),sharex=True)
for i in range(len(k_bind_c_range)):
    ax[i].plot(np.arange(1,n_clust+1),p_distribution_range[i]/p_distribution_range[i].max(),color=cols[i])
# ax.set(xscale="log")
    ax[i].spines['top'].set_visible(False)
    ax[i].spines['right'].set_visible(False)
    # if i!= (len(k_bind_c_range) -1):
    #     ax[i].set_xticks([])
    ax[i].set(xlim=(1,35))
    ax[i].set_ylabel(r"$%.2f$"%k_bind_c_range[i])


fig.text(0.04, 0.5, r'$k_{bind}$', va='center', ha='center', rotation='vertical')

fig.subplots_adjust(left=0.3)
fig.savefig("plots/k_bind vs p_distribution.pdf")

fig, ax = plt.subplots(len(k_bind_c_range))
for i in range(len(k_bind_c_range)):
    ax[i].plot(np.arange(1,n_clust+1),p_distribution_range[i]*np.arange(1,n_clust+1),color=cols[i])
# ax.set(xscale="log")
fig.show()
