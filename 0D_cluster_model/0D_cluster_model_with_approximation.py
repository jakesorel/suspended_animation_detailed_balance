...
class ZeroDClusterApprox(eqx.Module):
    params: float
    ...
    def f(self, t, y,params):
        ...
        j = np.arange(300) + 1
        pp = pi0 * (i0 / (i0 + j)) ** (1 / 3) * jnp.exp(
            j * jnp.log(K*p1 / i0 ** (1 / 3)) - 2 / 3 * jnp.cumsum(jnp.log((i0 + j))))


        fw_rate_per = K*(j/i0)**(1/3)
        rv_rate_per = Kr

        fw_rate_cluster = pp*fw_rate_per*p1
        rv_rate_cluster = pp*j*rv_rate_per
        net_rate_cluster = fw_rate_cluster - rv_rate_cluster

        fw_rate_monomers = y*kbind*p1*i
        rv_rate_monomers = y*i*kunbind

        fw_rate = fw_rate_monomers[:i0-1]
        rv_rate = rv_rate_monomers[1:i0]
        net_rate = fw_rate - rv_rate

        dty = jnp.concatenate((jnp.array((0.,)),net_rate)) - jnp.concatenate((net_rate,(jnp.array((0.,)))))
        dty = dty.at[0].add(-net_rate.sum() - net_rate_cluster.sum())
...
y0_new = sol1.y[:,-1][:3]
model_approx = ZeroDClusterApprox(params)
jac_approx = jit(jacrev(model_approx,argnums=[1,]))
sol_approx = solve_ivp(model_approx,tspan,y0_new,method="LSODA",t_eval=teval,jac=jac_approx,args=(params,))


fig, ax = plt.subplots()
ax.plot(sol.y[0])
ax.plot(sol_approx.y[0])
fig.show()
