...
def fit_params(log2dm_early_late,k_offF_eff,k_offB_eff,MC_early,MC_late,phi_late_true,k_bind,k_unbind_early,m0,k_unbind_anox,A_tot, B_tot,inv_kappa_B_anox, n, psi,rho0):
    ...
    class Eqx_f(eqx.Module):
        ...
        def f_anox_hss(self, t, y):
            ...
            k_onF, k_offF, k_onB, k_offB, inv_kappa_B, k_nuc, k_diss, F0, k_unbind_late = fit_params
    ...
    n_repeat = 5
    log_fit_params_save = np.zeros((n_repeat,9))
    for repeat in range(n_repeat):
        #np.log((1e-2, 1e-2, 1e-2, 1e0,1e-3, 1e-1, 1e-4, 1e-4,1e-3, 1e0))
        log_fit_params0 = jnp.array(np.random.uniform(-4,0,9))
